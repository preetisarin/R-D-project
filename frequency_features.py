...
class Freq_feature():
    ...
    def MFCC(self,data,sampling_rate):
        Mfcc=l.feature.mfcc(y=data, sr=sampling_rate)
    ...
    def compute_filepath_mfcc(self,directory,frame_length,overlap,storing_path='./frequency/'):
            ...
            # print(root,dirs,files)
                for file in files:
                    ...
                    if file.endswith('.csv'):
                        ...
                        self.extract_freq_mfcc(filepath,frame_length,overlap,storing_path)
    def compute_filepath_psd(self,directory,frame_length,overlap,storing_path='./frequency/'):
        for root, dirs, files in os.walk(directory):
            # print(root,dirs,files)
                for file in files:
                    print(file)
                    if file.endswith('.csv'):
                        filepath=os.path.join(root,file)
                        self.extract_freq_psd(filepath,frame_length,overlap,storing_path)
 
    def extract_freq_mfcc(self,filepath,frame_length, overlap,storing_path='./frequency/'):
        ...
        for i,val in enumerate(subsection_name):
            ...
            # for i in range(0,len(se8_subdata),1):
                ...
                f=list(self.MFCC_1(data,sampling_rate).flatten())
                ...
                with file:
                    ...
                    write.writerow(f)
    def find_peak(self,freq, data):
        '''
            Find peaks of power spectrum density with height greater than 0.00000001
        '''
        col=[]
        ...
        return col
    def extract_freq_psd(self,filepath,frame_length, overlap,storing_path='./frequency/'):
        '''
            Compute power spectrum density for acoustic signal. 
            frame_length=0.06 sec
            Overlap= 50%
        '''
        print(filepath)
        df=pd.read_csv(filepath)
        ...
        for i,val in enumerate(subsection_name):
            ...
            # for i in range(0,len(se8_subdata),1):
                ...
                f=list(self.find_peak(freq_data,data))
                # f=list(self.MFCC_1(data,sampling_rate).flatten())
